...
// 图片上传功能
     ...
     // 禁掉全局的拖拽功能。这样不会出现图片拖进页面的时候，把图片打开。
        ...
        server: BASE_URL+'/report/event/uploadFilesEventByMass',
        fileNumLimit: 300,
    ...
    uploader.on( 'uploadSuccess', function( file,response ) { 
       // path.push(response);
        //console.log(response);//这段代码根据需要选择是否写，也就是这句话可有可无
        responseMass = response ;
    });
    ...
    uploader.on( 'uploadBeforeSend', function( object, data,header ) {
        ...
        // 修改data可以控制发送哪些携带数据。
        data.eventCarNo = $("#eventCarNo").val().trim();
    ...
    $("#uploadMessage").click(function(){
       ...
       //这里判断是否按要求选择数据
    	 ...
    	 if(field_IsEmpty(eventDescription)){
    		 ...
    		 return;
    		 alert("123142");
    	    	alert(responseMass.fileDirPath);
    	    	var getPath = responseMass.fileDirPath ; 
    	    	//获取需要上传的信息
    	         var sfiles="";
    	    
    	    	 var files=uploader.getFiles();
    	    	 console.log(files[0].size)
    	    	 console.log("文件的数量："+files.length);
    	    	 for (var i = 0; i < files.length; i++) {
    	    		 var name = files[i].name ;
    	    		 var size = files[i].size ;
    	    		 if(sfiles==""){
    	    			 sfiles = getPath + size + "_"+name ;
    	    		 }else{
    	    			 sfiles = sfiles+","+getPath+ size + "_"+name ;
    	    		 }
    			}}

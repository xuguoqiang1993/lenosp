...
jQuery(function() {
    ...
    uploader = WebUploader.create({
     ...
     // 禁掉全局的拖拽功能。这样不会出现图片拖进页面的时候，把图片打开。
        ...
        server: BASE_URL+'/report/event/uploadFilesEventByTraffic',
        fileNumLimit: 300,
    ...
    $upload.on('click', function() {
     ...
     var eventRoad=$("#eventRoad").val().trim(); //违法地点
   	 ...
   	 var eventSuggest=$("#eventSuggest").val().trim();  //隐患建议
   	if(field_IsEmpty(eventRoad) || eventRoad=="请选择"){
		 swal("提示","请选择违法地点","info");
		 return;
	 }
	 
	 if(field_IsEmpty(eventRunKm)){
		 swal("提示","请输入公里桩(KM)","info");
		 return;
	 }
	 
	 if(field_IsEmpty(eventRunM)){
		 swal("提示","请输入公里桩(M)","info");
		 return;
	 }
	 ...
	 if(field_IsEmpty(eventDescription)){
		 swal("提示","请填写隐患描述","info");
		 return;
	 }
	 
	 if(field_IsEmpty(eventSuggest)){
		 swal("提示","请填写意见与建议","info");
		 return;
	 }
    	//判断有没有选择图片
    ...
    uploader.on( 'uploadBeforeSend', function( object, data,header) {
        ...
        // 修改data可以控制发送哪些携带数据。
     var eventRoad=$("#eventRoad").val().trim(); //道路
   	 var eventRunKm=$("#eventRunKm").val().trim(); //KM公里桩
   	 var eventRunM=$("#eventRunM").val().trim();  //M公里桩
        
   	 data.eventRoad = eventRoad ;
   	 data.eventRunKm = eventRunKm ;
   	 data.eventRunM = eventRunM ;
    ...
    function uploadMessage(sfiles){
    	...
    	var eventRoad=$("#eventRoad").val().trim(); //违法地点
    	 ...
    	 $.ajax({
    			type : "POST",
    			url : BASE_URL+"/report/event/insertHiddenDangerDecord",
    			data : {
    			 "userId":BASE_USERID,
    			...
    			success : function(json) {
    				 if(!field_IsEmpty(json)){
    					 if(json.upload=="1"){
   				    	  swal('提交成功!','隐患事件已经提交成功,页面跳转中....','success'); 
				    	  // 跳转到页面

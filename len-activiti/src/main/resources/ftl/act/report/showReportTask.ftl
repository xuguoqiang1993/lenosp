  ...
  layui.use('table', function () {
    ...
    table.render({
      ...
      , cols: [[
        ...
        , {field: 'createTime', title: '创建时间', width: '15%', sort: true,templet: '<div>{{ layui.laytpl.toDateString(d.createTime,"yyyy-MM-dd HH:mm:ss") }}</div>'}
    ...
    table.on('tool(task)', function (obj) {
      var data = obj.data;
      if (obj.event === 'handle') {
        laypoenmax('办理','${re.contextPath}/report/agent/'+data.id,'task-agent');
